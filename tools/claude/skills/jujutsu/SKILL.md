...
description: >
  ...
  to jj v0.36. Raw `git` commands in a non-colocated jj repo can corrupt state, so always
...
**Pinned version:** jj v0.36. Some flag names changed in this release (notably
`--destination`/`-d` → `--onto`/`-o` for `rebase`, `split`, `revert`). See
`references/version-notes.md` for the full v0.36 delta.
...
- **`references/version-notes.md`** — jj v0.36-specific behavior, deprecations from
  v0.35 → v0.36, flag renames (especially `--destination`/`-d` → `--onto`/`-o`), and
  the migration notes that matter for muscle memory. Load when commands are erroring
