...
metadata:
  hermes:
    related_skills: [parallel-research-workers, sprint-execute, sprint-retrospective, writing-plans]
---
...
**Ledger script path resolution caveat:** the script's `project_root()` hard-codes `parents[4]` assuming `<project>/.claude/skills/sprint-planner/scripts/ledger.py`. When the skill is installed globally at `~/.hermes/skills/software-development/sprint-planner/scripts/ledger.py`, that resolver points at `~/.hermes` instead of the user's actual project, and the ledger gets written to the wrong tree. Detection: `python3 .claude/skills/sprint-planner/scripts/ledger.py list` returns either an unrelated ledger or "file not found" when you expected to see this project's sprints. Workaround: manage the ledger by hand for this run — write `docs/sprints/ledger.yaml` directly in the project, format documented in `references/ledger-yaml-format.md`. Real fix would be teaching the script to walk up looking for a `docs/sprints/` marker rather than relying on a fixed depth. Confirmed on <INTERNAL-TICKET> (2026-05-11) where the skill lives at `~/.hermes/skills/` and the project workspace is `~/Work/DEVX/<INTERNAL-TICKET>/`.
...
**Use the `parallel-research-workers` skill for the actual dispatch shape, failure modes, and web-UI fallback recipe.** That skill is the canonical home for cross-CLI orchestration; this skill should not duplicate its content. Load `parallel-research-workers` before firing the three drafters — it covers codex auth-token recovery, gemini quota/preview-model 429s, the `delegate_task` Claude leg's web-toolset trap, and the gemini-Deep-Research browser fallback (which is often *better* than the CLI, per <INTERNAL-TICKET>).

The YOLO / non-interactive invocations:

- **codex**: `codex exec --dangerously-bypass-approvals-and-sandbox "<prompt>"` (or `codex exec --skip-git-repo-check --sandbox workspace-write -C <dir> "<prompt>"` if the worktree isn't a git repo). After re-auth via `codex login`, the Hermes-profile form `hermes -p researcher-codex chat -q "<prompt>"` is more reliable than re-running `codex exec` directly — it doesn't share the direct-CLI refresh-token corruption state.
- **gemini**: `gemini -y -p "<prompt>"`. If you hit `gemini-3-flash-preview` capacity 429s (server-side, not your quota), retry with `-m gemini-2.5-pro` or fall back to web-UI Deep Research per `parallel-research-workers/references/dispatch-postmortems.md`.
- **claude**: If you (the orchestrating session) ARE Claude, **use `delegate_task` for the Claude leg, not `claude -p`** — you can't fan out a sibling Claude process from inside a Claude session cleanly. `delegate_task(toolsets=["web", "file"])` works for drafting (no tool_trace pitfalls since the drafter just reads local files and writes one). When the orchestrator is NOT Claude, use `claude -p --permission-mode acceptEdits "<prompt>"`.
...
- **Draft failures.** If one agent's CLI call fails (rate limits, network), surface that to the user and offer three options: (a) retry that one agent (after the user fixes the underlying issue — re-auth, wait for quota, etc.); (b) backfill with a web-UI fallback (paste the drafter prompt into the provider's web UI, save the result to `drafts/SPRINT-XXXX-<AGENT>.md` by hand — see `parallel-research-workers/references/dispatch-postmortems.md` for the prompt-transform pattern); (c) proceed with two drafts. Don't silently drop. The web-UI fallback is often the right answer when the same provider is hitting CLI-specific issues (e.g. gemini-3-flash-preview server-side capacity 429s) but the provider's hosted UI is healthy — and for Gemini specifically, Deep Research in the web UI is often higher-quality than the CLI even when both work. Same pattern for the cross-critique phase: if a CLI is down for drafting, it's usually still down for critiquing, so plan to use the same web-UI fallback there. <INTERNAL-TICKET> used this for both Gemini phases.
...
- **Cross-critique CANNOT validate CLI surfaces.** When drafters disagree about whether a CLI flag, subcommand, or config field exists (e.g. one says `--tenant` exists, another says it's hallucinated), the cross-critique step is unable to resolve it — peer reviewers are working from the same training data and confidently support whichever side matches their priors. The dispute MUST be deferred to an empirical probe task in the merged plan. Concrete planner rule: when a CLI-flag dispute surfaces in cross-critique, the merged plan's first wave includes a `validate_<cli>_affordances.py` task that runs `<cli> <subcmd> --help` against the real binary and writes the surface to a version-controlled `docs/<topic>/<cli>-affordances.md` table; all downstream tasks that depend on the disputed flag gate on this artifact. Worked example: SPRINT-0001 (<INTERNAL-TICKET>) had a Claude-critic confidently asserting `hermes kanban create --tenant`/`--assignee` were Gemini hallucinations; the empirical probe (T-002 + T-003 of the merged plan) confirmed both flags exist in v0.13.0. Without the gating task, the implementer would have built wrappers for nonexistent gaps. Treat any drafter-vs-critic CLI-flag fight as a forcing function for an empirical-probe gate.
