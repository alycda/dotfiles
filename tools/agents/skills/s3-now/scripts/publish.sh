#!/usr/bin/env bash
# Upload a file (or dir) to the private s3-now bucket and print a pre-signed URL.
# The bucket is never public: generating a URL requires a live SSO session,
# and the URL itself is the time-limited grant you hand to viewers.
#
...
#
# Expiry: raw seconds, or Nm/Nh/Nd (max 7d — S3 hard cap). NOTE: a pre-signed
# URL made with SSO temp credentials dies when those credentials expire, if
# that is sooner than --expires. For the longest-lived links, presign right
# after a fresh `aws sso login`.
#
# State: .s3now/state.json in the working directory (slug -> key map).
set -euo pipefail

CONFIG="$HOME/.s3now/config.json"
STATE_DIR=".s3now"
...
AWSP=(aws --profile "$PROFILE" --region "$REGION")
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --profile)   PROFILE="$2"; AWSP=(aws --profile "$PROFILE" --region "$REGION"); shift 2 ;;
    -*)          die "unknown flag: $1" ;;
    *)           SRC="$1"; shift ;;
  esac
done
...
if $PRESIGN_ONLY; then
  ...
  echo "presign_result.expires_in=${EXPIRES}s (or when the SSO session's credentials expire, if sooner)" >&2
  presign "$KEY"
  exit 0
fi
...
echo "publish_result.url_expires_in=${EXPIRES}s (or when the SSO session's credentials expire, if sooner)" >&2
presign "$KEY"
