...
description: >
  ...
  whenever the user says "task failed successfully", "well that didn't work", "here's what went
  wrong", "I broke it on purpose", "log this failure", or "I learned the hard way". Also trigger
  when they paste a wall of debug notes, describe an approach they abandoned, or say "I want to
...
Helps an engineer capture failures as evidence of how they learn — by breaking things
intentionally, stress-testing assumptions, and updating their mental model when reality
pushes back.

**Core principle:** The failure was load-bearing. It couldn't have been learned any other way.
Write for someone who needs to understand what changed in the author's thinking, not just
what broke.

**Extraction filter:** Only log it if it changed the author's mental model. "It just didn't
work" goes in the trash. "I now understand WHY it doesn't work, and here's what I do
differently" goes here.
...
→ Ask 2–3 targeted clarifying questions. The key question is always: "What do you know now
  that you didn't know before?" If they can't answer that, it's not a failure doc entry yet.
...
### 1. Intentional Break
They poked it on purpose to see what would happen. The failure was the experiment.
- Includes: stress tests, edge case probing, "what happens if I do X wrong" explorations
- Entry format: What they did → what they expected → what actually happened → what that revealed

### 2. Assumption Collapse
They believed X. Reality said no. They now believe Y.
- Includes: wrong mental models, misread docs, false analogies from another language/system
- Entry format: What they assumed → what evidence contradicted it → what the correct model is
...
- Entry format: What the gap was → who or what was affected → what they changed or advocated for

### 4. Judgment Call That Missed
They made a call. They got feedback. They updated.
- Includes: architectural decisions that got pushback, proposals that didn't land, tech bets
  that didn't pay off
- Entry format: What they proposed/decided → what the feedback was → what they now weigh
...
mark it with 🔥. These are the entries that read as next-level signal: someone who learns by
...
If manager-ready framing is requested:
...
If exec framing is requested:
- One sentence on what broke, two on what changed in how the author approaches the problem space
...
When asked to draft a narrative section (e.g., "write up my learning velocity section"),
...
time?" If they still can't answer, flag it as pending and revisit.
...
framing; flag "also brag-doc eligible" with a one-line summary of the impact angle, and let the
author decide whether to cross-log. If a brag doc with a Learning Log exists, the full entry
lives here — the Learning Log gets a summary line and a pointer, not a duplicate.

**Emotional/venting tone in input:** Extract the factual core and the mental model shift.
Don't reproduce the tone. If the frustration is evidence (they had to fight through something
