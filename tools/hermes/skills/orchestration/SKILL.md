...
| Specialist skills installed in target profile(s) | `~/.hermes/skills/sprint/sprint-{planner,execute,retrospective}/` | Yes — see Setup below |
...
Build a `sprint-manifest.yaml` in `docs/sprints/manifests/<parent-id>-<timestamp>.yaml` (see `templates/sprint-manifest.yaml.example`). Include sprint slug, ticket list, total estimate, SDK targets, and `blocked_by` relationships derived from Linear's `blocks`/`blockedBy` graph.
...
1. **Specialist skills must be in Hermes form.** Copy your existing `~/.claude/skills/sprint-{planner,execute,retrospective}/` into `~/.hermes/skills/sprint/` (preserving structure). They work in Hermes unchanged because they shell out to `codex` / `gemini` / `claude` CLIs the same way Claude Code does. See `references/kanban-context-additions.md` for optional kanban-aware additions to each.
